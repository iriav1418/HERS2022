...
## Summarize sample size for Herbicide Treatments and Rhizobial Strain Inoculation
aggregate(data = nod_in_dat, first_nodule_timing ~ dicamba_trt, FUN = length) # N = 206 for control-treated plants, N = 202 for dicamba-treated plants

aggregate(data = nod_in_dat, first_nodule_timing ~ strain_ID, FUN = length) # N = ~50 per rhizobial strain inoculation
...
nod_time_A <- ggplot(data = nod_in_fit_dat, aes(x = time, y = surv2, linetype = dicamba_trt2)) +
  ...
  labs(y = "Proportion of Nodulated Plants", x = "Weeks Post-Inoculation", linetype = "Herbicide \nTreatment") +
...
nod_timeB <- ggplot(data = nod_in_fit_dat2, aes(x = time, y = surv2)) +
  ...
  labs(y = "Proportion of Nodulated Plants", x = "Weeks Post-Inoculation") +
...
### Model selection. First, determine whether it is better to analyze the data using a linear mixed model (LMM) with log-transformed data or generalized linear mixed model (GLMM) with a Poisson or negative binomial distribution


nod_num_lmer_mod1 <- lmer(log(WK4_num_nodules) ~ dicamba_trt*plant_genotype*strain_ID + (1|Tray) + WK4_fungal_pathogen_minor + temporal_block, data = nod_num_dat)

nod_num_glmer_mod1 <- glmer(WK4_num_nodules ~ dicamba_trt*plant_genotype*strain_ID + (1|Tray) + WK4_fungal_pathogen_minor + temporal_block, family = poisson, glmerControl(optimizer="bobyqa", optCtrl = list(maxfun = 100000)), data = nod_num_dat)

nod_num_glmer_mod2 <- glmer.nb(WK4_num_nodules ~ dicamba_trt*plant_genotype*strain_ID + (1|Tray) + WK4_fungal_pathogen_minor + temporal_block, glmerControl(optimizer="bobyqa", optCtrl = list(maxfun = 100000)), data = nod_num_dat)

simulationOutput <- simulateResiduals(fittedModel = nod_num_lmer_mod1, plot = F)
plot(simulationOutput) # no deviations detected

simulationOutput <- simulateResiduals(fittedModel = nod_num_glmer_mod1, plot = F)
plot(simulationOutput) # deviations detected

simulationOutput <- simulateResiduals(fittedModel = nod_num_glmer_mod2, plot = F)
plot(simulationOutput) # deviations detected

## LMMs with log-transformed data are preferred in this case as they pass the DHARMa diagnostic tests better than the Poisson or negative binomial GLMMs. Now we will choose from mixed effects linear models to analyze log-transformed nodule number data to evaluate random and fixed effect factors. 
...
### Test Hypotheses
nod_num_lmer_mod3 <- lmer(log(WK4_num_nodules) ~ dicamba_trt*plant_genotype + dicamba_trt*strain_ID + plant_genotype*strain_ID + (1|Tray) + WK4_fungal_pathogen_minor + temporal_block, data = nod_num_dat, REML = TRUE) # run model with REML= TRUE. In the book Mixed Effects Models and Extensions in Ecology with R (Zuur et al. 2009), it is recommended to compare models with REML set to FALSE as done above, then to do hypothesis testing on the best fitting model with REML set to TRUE. 
...
summary(nod_num_emmeans, by = NULL, adjust = "dunnettx")# Dunnet p-value adjustments, these results are reported in the supplement
...
### Test hypotheses
N_model4 <- lmer(delta_15N ~ dicamba_trt*strain_ID + dicamba_trt*plant_genotype + (1|Tray) + WK4_fungal_pathogen_minor, REML = TRUE, data = N_data4)
...
N_per_nod_model <- lmer(delta_15N ~ dicamba_trt*strain_ID + dicamba_trt*plant_genotype + (1|Tray) + WK4_fungal_pathogen_minor + WK6_num_nodules, REML = TRUE, data = N_data4)
Anova(N_per_nod_model, type = 3) #sig effects of fungal pathogen, and dic x strain, non sig effect of nodule number
...
### Test hypotheses
sb_lmer_mod4 <- lmer(sqrt(shoot_biomass) ~ strain_ID*dicamba_trt + dicamba_trt*plant_genotype + (1|Tray), data = sb_dat, REML = TRUE)
...
WK6_rhiz_eff_dat$plant_genotype2 <- "X"
for(i in 1:nrow(WK6_rhiz_eff_dat)){
  if(WK6_rhiz_eff_dat[i,3] == "kenland"){
    WK6_rhiz_eff_dat[i,11] <- "Kenland"
  }
  if(WK6_rhiz_eff_dat[i,3] == "mammoth"){
    WK6_rhiz_eff_dat[i,11] <- "Mammoth"
  }
  
  i <- i + 1
}

WK6_rhiz_eff_plot <- ggplot(WK6_rhiz_eff_dat, aes(x = dicamba_trt2, y = WK6_rhizobial_effect, color = strain_ID2))+
  geom_point(size = 3, position = position_dodge(width = 0.3)) +
  geom_errorbar(aes(ymin = WK6_rhizobial_effect-se, ymax = WK6_rhizobial_effect + se), width = 0, size = 1, position = position_dodge(width = 0.3)) +
  geom_line(aes(group = strain_ID2), position = position_dodge(width = 0.3), size = 1) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  labs(x = "Herbicide Treatment", y = "Rhizobial Effect on Plant Size (mg)", color = "Strain") +
  scale_color_manual(values = c8) +
  theme_classic() +
...
new.tree <- bind.tip(tree = rhizobia.tree,
                     tip.label = nod_num_phylo_dat[[15]][1], # sampleID reference - [[1]] gives the column [] gives the 
...
for (i in 2:nrow(nod_num_phylo_dat)) {
  new.tree <- bind.tip(tree = new.tree,
                       tip.label = nod_num_phylo_dat[[15]][i], # Tip labels are pulled from the
                       edge.length = 0.0001,
                       position = 0,
...
### Figure 3: rhizobial effect on plant growth results
WK6_rhiz_eff_plot
